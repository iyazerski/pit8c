...
def process_annual_report(broker: SupportedBroker, tax_report_file: Path) -> None:
    ...
    if not tax_report_file.exists():
        raise Pit8cError(f"'{tax_report_file}' does not exist")
    ...
    if not trades:
        raise Pit8cError(f"'{tax_report_file}' does not contain any trades")

    # apply FIFO logic
    closed_positions = match_trades_fifo(trades)
    if not closed_positions:
        raise Pit8cError(f"'{tax_report_file}' does not contain any closed positions")
